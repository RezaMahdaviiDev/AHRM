...
func (e *Engine) CalculateAll(options []sourcearena.Option, underlyingPrice float64, kind FilterKind) []Spread {
	...
	for _, opt := range options {
		...
		byExpiry[opt.ExpiryDate].calls = append(byExpiry[opt.ExpiryDate].calls, opt)
	}

	var out []Spread
	for _, g := range byExpiry {
		sort.Slice(g.calls, func(i, j int) bool {
			return g.calls[i].StrikePrice < g.calls[j].StrikePrice
		})

		for i, k1Opt := range g.calls {
